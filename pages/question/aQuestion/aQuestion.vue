<template>
	<view>
		...
		<view class="page-box">
			<view class="order">
				<view class="item" >
					...
					<view class="content">
						<view class="title u-line-2">{{ question.title }}</view>
						<view class="delivery-time">{{ question.content }}</view>
					</view>
				</view>
				<view class="bottom" style="margin: 0;" @click="gotoAdd()">
					<!-- <view class="logistics btn">查看物流</view> -->
					<view class="add-btn"><u-icon name="edit-pen-fill"></u-icon>写回答</view>
					<!-- <view class="exchange btn">更改</view> -->
					<!-- <view class="more">
		...
		<view class="wrap">
			<swiper class="swiper-box" @transition="transition" >
						...
						<view class="page-box">
							<view class="order" v-for="(res, index) in dataList" :key="res.id">
								<view class="item" :key="index">
									...
									<view class="content">
										<view class="title">{{ res.answer }}</view>
										<!-- <view class="delivery-time">发货时间 {{ res.deliveryTime }}</view> -->
									</view>
								</view>
								<view class="bottom">
									<!-- <view class="logistics btn">查看物流</view> -->
									<view class="evaluate btn" @click="likeAnswer(res.id)"><u-icon name="thumb-up"></u-icon>{{res.likes}}</view>
									<!-- <view class="exchange btn">更改</view> -->
									<!-- <view class="more">
										<u-icon name="trash" color="rgb(203,203,203)" size="50"></u-icon>
									</view> -->
...
<script>
	import { getQuestion, likes,getQAnswer } from '../../../api/question.js'
	export default {
		data() {
			return {
				title: '问题',
		...
		onLoad(option) {
			this.question.id = option.id
			console.log(this.question.id)
			this.getQuestion(this.question.id)
			this.getQAnswer(this.question.id)
		},
		methods: {
			getQuestion(id){
			...
			getQAnswer(questionId){
				getQAnswer(questionId).then(res => {
			...
			gotoAdd(id){
				uni.navigateTo({
					url: '../addAnswer/addAnswer?id=' + this.question.id,
					success: (res) => {
						console.log(res)
					},
					fail: (err) => {
...
<style lang="scss" scoped>
	...
	.add-btn {
		...
		line-height: 60rpx;
		text-align: center;
		color: #f3d2b8; //$u-type-warning-dark
		border-top:1rpx solid  #f3d2b8;
	}
</style>
