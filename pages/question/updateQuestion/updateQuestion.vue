		...
		<view class="acontent">
			<u-form :model="model" :rules="rules" ref="uForm" :errorType="errorType">
				<u-form-item :leftIconStyle="{color: '#888', fontSize: '32rpx'}" label-width="120" :label-position="labelPosition"
				 label="标题" prop="title">
					<u-input style="border: 1px solid $uni-text-color-grey;" v-model="model.title" type="text"></u-input>
				</u-form-item>
				...
				<u-form-item :leftIconStyle="{color: '#888', fontSize: '32rpx'}" label-width="120" :label-position="labelPosition"
				 label="内容" prop="title">
					...
					<textarea class="ql-container" v-model="model.content" />
					</u-form-item>
...
<script>
	import { getQuestion,updateQuestion } from '../../../api/question.js'
	export default {
		data() {
			return {
				add_title: '添加文章',
			...
			submit() {
				...
				this.$refs.uForm.validate(valid => {
					...
					if (valid) {
						...
						updateQuestion(this.requestParameters).then(res => {
							console.log(res)
						}).catch(res => {
							console.log(res)
						})
