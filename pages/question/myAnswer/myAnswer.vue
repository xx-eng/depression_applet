						...
						<view class="page-box">
							<view class="order" v-for="(res, index) in dataList" :key="res.id">
								<view class="item" :key="index">
									...
									<view class="content">
										<view class="title u-line-2">{{ res.answer }}</view>
								...
								<view class="bottom">
									...
									<view class="exchange btn" @click="gotoUpdate(res.id)">更改</view>
									<view class="more" @click="deleteAnswer(res.id)">
...
<script>
	import {getMyAnswer, deleteAnswer} from '../../../api/question.js'
	...
	export default {
		...
		methods: {
			getMyAnswer() {
				// user.state.userId
				console.log("user:", user.state.userId)
				getMyAnswer(user.state.userId).then( res => {
