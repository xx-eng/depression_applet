		...
		<view class="acontent">
			<u-form :model="model" :rules="rules" ref="uForm" :errorType="errorType">
				<!-- <u-form-item :leftIconStyle="{color: '#888', fontSize: '32rpx'}" label-width="120" :label-position="labelPosition"
				 label="标题" prop="title">
					<u-input style="border: 1px solid $uni-text-color-grey;" placeholder="请输入标题" v-model="model.title" type="text"></u-input>
				</u-form-item> -->
				...
				<u-form-item :leftIconStyle="{color: '#888', fontSize: '32rpx'}" label-width="120" :label-position="labelPosition"
				 label="内容" prop="content">
					<!-- <editor id="editor" class="ql-container" placeholder="请输入文章内容或图片简介" @ready="onEditorReady" v-model="model.contentHtml"></editor> -->
					<textarea class="ql-container" v-model="model.content" placeholder="请输入动态内容" />
					</u-form-item>
...
<script>
	import { addReply } from '../../../api/posts.js'
	...
	export default {
		...
		onLoad(option) {
			this.model.postsId = option.postsId
			this.model.userId = option.userId
			...
			submit() {
				...
				this.$refs.uForm.validate(valid => {
					...
					if (valid) {
						...
						const user = {
							criticId: this.userId
						}
						this.requestParameters = Object.assign({}, user, this.model)
						console.log('requestParam', this.requestParameters)
						addReply(this.requestParameters).then(res => {
							console.log(res)
						}).catch(res => {
							console.log(res)
						})
